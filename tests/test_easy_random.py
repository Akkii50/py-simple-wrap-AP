...
def test_flip_coin_returns_heads_or_tails():
    outcomes = {flip_coin() for _ in range(80)}
    assert outcomes.issubset({"Heads", "Tails"})
    assert outcomes  # at least one outcome was produced
...
def test_pick_random_item_rejects_empty_sequence():
...
def test_random_bool_returns_bool_and_can_be_forced(monkeypatch):
